...
import { GeminiWordRelationship } from "../../../services/GeminiWordRelationship";
import {
  Relationship,
  SuggestedRelationship,
  RelationshipType,
} from "../../../type/wiki";
import { EditExistingWordRelationshipSection } from "./EditExistingWordRelationshipSection";
import { EditSuggestWordRelationshipSection } from "./EditSuggestWordRelationshipSection";
...
interface EditWordRelationshipSectionProps {
  data: WordData;
...
export function EditWordRelationshipSection({
  data,
...
}: EditWordRelationshipSectionProps) {
  ...
  const handleCheckRelationship = async (
    ...
    try {
      const endpoint =
        type === "word" ? "/wiki/word/find" : "/wiki/phrases/find";
      const response = await apiService.get<any>(
        `${endpoint}?word=${encodeURIComponent(text)}`
      );
      const exists =
        response.code === 200 &&
        response.data &&
  ...
  const handleCreateBulkRelationships = async () => {
    ...
    try {
      ...
      await apiService.post(
        `/wiki/word/relationships/bulk/${data.id}`,
        reqBody
      );
  ...
  const handleRemoveRelationship = async (relationshipId: string) => {
    try {
      await apiService.delete(`/wiki/word/relationships/${relationshipId}`);
  ...
  const loadSuggestions = async () => {
    ...
    setIsLoading(true);
    try {
      const suggestions = await GeminiWordRelationship.suggestRelationships(
        data.id,
        data.word,
        existingRelationships
      );

...
import { Plus, RefreshCw, Trash2, Check, Settings2, Sparkles } from 'lucide-react'
import { SuggestedRelationship, RelationshipType } from '../../../type/wiki'
import { relationshipTypeLabels } from './EditPhraseRelationshipSection'
import { GeminiSetting } from '../../../../../componentsform/GeminiSettingForm'
...
interface Props {
  ...
  handleCheckRelationship: (text: string, type: 'word' | 'phrase', index: number) => void
  ...
  onQuickCreateWord: (text: string, cb?: () => void) => Promise<void>
  onQuickCreatePhrase: (text: string, cb?: () => void) => Promise<void>
  isQuickCreatingWord: boolean
  isQuickCreatingPhrase: boolean
  isLoading: boolean
}

export function EditSuggestPhraseRelationshipSection({
  ...
  const [holdingRefreshIndexes, setHoldingRefreshIndexes] = React.useState<Set<number>>(new Set())
  const [isAddingRelationships, setIsAddingRelationships] = React.useState(false)

  const holdRefresh = (idx: number) => setHoldingRefreshIndexes((prev) => new Set(prev).add(idx))
    ...
    <div>
        ...
        <div className="flex gap-2">
          ...
          {selectedRelationships.size > 0 && (
            ...
            >
              ...
              {isAddingRelationships ? 'Đang thêm...' : <>Thêm {selectedRelationships.size}</>}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {suggestedRelationships.length === 0 && (
          <div className="text-gray-400 px-2 italic text-base">Chưa có gợi ý nào.</div>
        )}
        {suggestedRelationships.map((relationship, index) => (
          <div
            ...
            className={`
              ...
              ${relationship.exists ? 'border-primary/10 bg-primary/5' : 'border-red-200 bg-red-50'}
                ...
                <div className="font-semibold text-gray-900 flex flex-wrap items-center gap-2 text-[15px]">
                  ...
                  <p className="text-xs">
                    {relationship.relationship_type in relationshipTypeLabels
                      ? relationshipTypeLabels[relationship.relationship_type as RelationshipType]
                          .label
            ...
            <div className="flex flex-col justify-center items-center ml-3 gap-0 h-full">
              {!relationship.exists && (
                <>
                  ...
                  <Button
                    onClick={() => {
                      handleCheckRelationship(relationship.text, relationship.related_type, index)
...
export default EditSuggestPhraseRelationshipSection

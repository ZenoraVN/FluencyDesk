...
import { GeminiWordService } from "@/presentation/pages/WikiManage/services/GeminiWordService";
...
export const WordRepetitionForm: FC<WordRepetitionFormProps> = ({
  ...
  const handleCheck = async () => {
    ...
    try {
      const result = await ApiService.get<any>(
        `/wiki/words/find-with-question?query=${encodeURIComponent(
          form.getValues("word")
        )}`
      );
  ...
  const handleQuickCreate = async () => {
    ...
    try {
      ...
      setCreateError(
        "Không thể tạo từ vựng. (Lỗi: " + (err?.message || "unknown") + ")"
      );
  ...
  const _normalizeWikiWordReq = (wordObj: any) => {
    ...
    if (Array.isArray(wordObj.definitions)) {
      wordObj.definitions = wordObj.definitions.map(
        (def: any, idx: number) => ({
          ...def,
          is_main_definition:
  ...
  const handleOpenWordDrawer = async () => {
    ...
    try {
      ...
      const response = await ApiService.get<WikiWordDetailResponse>(
        `/wiki/words/${wikiWordId}`
      );
      if (!ignore) {
                ...
                <FormItem>
                  ...
                  {fetchStatus === "not_found" && (
                    <div className="mt-2 text-yellow-700 text-xs">
                      Không tìm thấy từ này trong hệ thống. Bạn có thể tạo nhanh
                      từ vựng bằng AI.
                    </div>
                  )}
                  {createStatus === "error" && (

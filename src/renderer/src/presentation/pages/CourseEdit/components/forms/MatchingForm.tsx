      ...
      validate: {
        ...
        maxTen: (value: MatchingFormData["matchings"]) =>
          value && value.length <= 10
            ? true
            : "Chỉ được tối đa 10 cặp ghép nối",
        noDuplicateContent: (value: MatchingFormData["matchings"]) => {
          ...
          if (hasDuplicates(questions))
            return "Các câu hỏi không được trùng nhau";
          if (hasDuplicates(answers)) return "Các đáp án không được trùng nhau";
          if (hasDuplicates(explains))
            return "Các giải thích không được trùng nhau";
          ...
          for (let i = 0; i < value.length; i++) {
            ...
            if (!q || !a || !e) continue;
            if (q === a)
              return `Câu hỏi và đáp án trong cặp ${
                i + 1
              } không được giống nhau`;
            if (q === e)
              return `Câu hỏi và giải thích trong cặp ${
                i + 1
              } không được giống nhau`;
            if (a === e)
              return `Đáp án và giải thích trong cặp ${
                i + 1
              } không được giống nhau`;
          }
          return true;
  ...
  useEffect(() => {
    if (!initialized.current) {
      ...
      const hasInitialPairs =
        initialData &&
        Array.isArray(initialData.matchings) &&
        initialData.matchings.length > 0;
          ...
          <div className="flex justify-between items-center pb-2 border-b border-gray-200">
            ...
            {fields.length < 10 && (
              <Button
                type="button"
                onClick={() =>
                  append({ question: "", answer: "", explain: "" })
                }

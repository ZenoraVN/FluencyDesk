...
import { GeminiPhraseService } from '../../../WikiManage/services/GeminiPhraseService'
import ApiService from '../../../../../service/ApiService'
import { Loader2, Sparkles } from 'lucide-react'
import { ViewWordDrawer } from '../../../../pages/WikiManage/components/Word/View/ViewWordDrawer'
...
export const PhraseRepetitionForm: FC<PhraseRepetitionFormProps> = ({
  ...
  const handleCheck = async () => {
    ...
    try {
      const result = await ApiService.get<any>(
        `/wiki/phrases/find-with-question?query=${encodeURIComponent(form.getValues('phrase'))}`
      )
  ...
  const handleQuickCreate = async () => {
    ...
    try {
      ...
      const phraseData = await GeminiPhraseService.generatePhraseDetails(phraseInput)
      ...
      setCreateError('Không thể tạo phrase. (Lỗi: ' + (err?.message || 'unknown') + ')')
  ...
  const _normalizeWikiPhraseReq = (phraseObj: any) => {
    if (phraseObj.pronunciation) {
      phraseObj.pronunciation = phraseObj.pronunciation.replace(/\//g, '').trim()
    }
    if (Array.isArray(phraseObj.definitions)) {
      phraseObj.definitions = phraseObj.definitions.map((def: any, idx: number) => ({
        ...def,
        is_main_definition:
  ...
  const handleOpenDrawer = async () => {
    ...
    try {
      ...
      const response = await ApiService.get<WikiPhraseDetailResponse>(`/wiki/phrases/${phraseId}`)
      if (!ignore) {
  ...
  return (
    <>
          ...
          <div className="rounded-lg space-y-4">
            <FormField
              ...
              render={({ field, fieldState: { error } }) => (
                <FormItem>
                  <FormLabel className="text-[#2D3748] font-medium">Cụm từ cần luyện tập</FormLabel>
                  <FormControl>
                    <CustomInput
                      value={field.value || ''}
                  ...
                  {fetchStatus === 'not_found' && (
                    <div className="mt-2 text-yellow-700 text-xs">
                      Không tìm thấy cụm từ này trong hệ thống. Bạn có thể tạo nhanh cụm từ bằng AI.
                    </div>
                  )}
                  {createStatus === 'error' && (
      ...
      {showDrawer && drawerData && (
        <ViewWordDrawer open={showDrawer} onClose={() => setShowDrawer(false)} data={drawerData} />
      )}
    </>
  )
}

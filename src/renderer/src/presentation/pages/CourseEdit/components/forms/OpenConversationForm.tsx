...
export const OpenConversationForm: FC<OpenConversationFormProps> = ({ initialData }) => {
  ...
  useEffect(() => {
    if (initialData?.example_conversation) {
      ...
      form.setValue('open_conversation.example_conversation', initialData.example_conversation)
    }
    if (initialData?.title) form.setValue('open_conversation.title', initialData.title)
    if (initialData?.overview) form.setValue('open_conversation.overview', initialData.overview)
  ...
  const updateConversation = (newMessages: Message[]) => {
    ...
    form.setValue('open_conversation.example_conversation', stringifyConversation(newMessages))
  }
      ...
      <div className="space-y-6">
        ...
        <FormField
          ...
          rules={{
            ...
            validate: (value) => (!value?.trim() ? 'Vui lòng nhập tiêu đề' : true)
          }}
          render={({ field, fieldState: { error } }) => (
            <FormItem>
              <FormLabel className="text-[#2D3748] font-medium">Tiêu đề chủ đề</FormLabel>
              <FormControl>
                <CustomInput
                  value={field.value || ''}
                  onChange={(val) => {
                  ...
                  className={`bg-transparent w-full ${
                    error ? 'border-red-500' : 'border-[#52aaa5] focus:border-[#52aaa5]'
                  }`}
                />
              </FormControl>
        ...
        <FormField
          ...
          rules={{
            ...
            validate: (value) => (!value?.trim() ? 'Vui lòng nhập tổng quan' : true)
          }}
          render={({ field, fieldState: { error } }) => (
            <FormItem>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center pb-1">
            <h4 className="text-sm font-medium text-[#2D3748]">Hội thoại mẫu</h4>
          </div>
          {messages.length === 0 && (
        ...
        <div className="rounded-lg border p-4 bg-blue-50">
          <p className="text-sm text-blue-600">
            Lưu ý: Đây là dạng câu hỏi mở, học viên sẽ được tự do thể hiện quan điểm của mình về chủ
            đề. Hội thoại mẫu chỉ mang tính chất gợi ý và không phải là khuôn mẫu cố định.
          </p>
        </div>
      </div>
    </Form>

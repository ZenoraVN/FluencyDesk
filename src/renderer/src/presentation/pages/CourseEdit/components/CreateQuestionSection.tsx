...
const QUESTION_TYPES_BY_SKILL: Record<SkillType, QuestionType[]> = {
  ...
  reading: ['fill_in_the_blank', 'choice_one', 'choice_multi', 'matching', 'true_false_not_given'],
  grammar: [
    'fill_in_the_blank',
    'choice_one',
    'choice_multi',
...
export const CreateQuestionSection: FC<CreateQuestionSectionProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedSkill === 'listening' && detailFormData && detailFormData.data) {
        questionData.listening_question_detail = detailFormData.data as ListeningDetailFormData
      }
      if (selectedSkill === 'reading' && detailFormData && detailFormData.data) {
        questionData.reading_question_detail = detailFormData.data as ReadingDetailFormData
      }
      ...
      if (selectedSkill === 'listening' && detailFormData && detailFormData.data) {
        ...
        if (listeningDetails.image_files && listeningDetails.image_files.length > 0) {
      ...
      if (selectedSkill === 'reading' && detailFormData && detailFormData.data) {
        ...
        if (readingDetails.image_files && readingDetails.image_files.length > 0) {
  ...
  const renderTypeForm = () => {
    ...
    switch (selectedType) {
      ...
      case 'word_repetition':
        return (
          <WordRepetitionForm lessonId={lessonId} sequence={questionSequence} onClose={onClose} />
        )
      case 'phrase_repetition':
        return (
          <PhraseRepetitionForm lessonId={lessonId} sequence={questionSequence} onClose={onClose} />
        )
        ...
        <div className="flex items-center gap-2">
          <div>
            <div className="text-lg text-black font-semibold">
              Lesson {lessonSequence}. {lessonTitle} - {selectedType} - question {questionSequence}
            </div>
          </div>
        </div>
        <button
          ...
          {isSubmitting && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-[9999] flex items-center justify-center">
              <div className="bg-white/90 backdrop-blur shadow-lg rounded-lg p-4 flex items-center gap-3">
                ...
                <span className="text-[#2D3748] font-medium">Đang tạo câu hỏi...</span>
              </div>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto space-y-6">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="text-base font-medium text-[#2D3748]">{skill.label}</h3>
                            <p className="text-sm text-[#718096] mt-1">{skill.description}</p>
                            <div className="text-sm text-[#718096] mt-2">
                              {metrics.totalQuestions.toLocaleString()} {METRIC_LABELS.questions}
                            </div>
                          </div>
                        </div>
            ...
            {selectedSkill && !selectedType && (
              <div className="space-y-4">
                {QUESTION_TYPES_BY_SKILL[selectedSkill].map((type) => {
                  ...
                  const metrics = TYPE_METRICS[type as keyof typeof TYPE_METRICS]
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-[#718096] mt-1">{typeDetails.overview}</p>
                          <div className="text-sm text-[#718096] mt-2">
                            {metrics.questionCount.toLocaleString()} {METRIC_LABELS.questions}
                          </div>
                        </div>
                        <div className="flex items-center gap-4 text-sm text-[#718096]">
                          <div className="flex items-center gap-1.5">
            ...
            {selectedSkill && selectedType && (
              <>
                ...
                {['word_repetition', 'phrase_repetition'].includes(selectedType) ? (
                  ...
                  <div className="flex gap-8 overflow-y-auto">
                    ...
                    <div className="flex-1 min-h-0 space-y-6 overflow-y-auto">
                      <h3 className="text-lg font-semibold text-[#2D3748]">Chi tiết câu hỏi</h3>
                      {renderTypeForm()}
                    </div>
                  </div>
                )}

...
import { useFormContext, useFieldArray, UseFormStateReturn } from 'react-hook-form'
import {
  Form,
  FormControl,
  FormField,
...
export const SentenceCompletionForm: FC<SentenceCompletionFormProps> = ({
  ...
  const { fields, append, remove } = useFieldArray<SentenceCompletionFormData>({
    ...
    rules: {
      ...
      validate: {
        maxTen: (value) => (value && value.length <= 10) || 'Chỉ được tối đa 10 câu hoàn thành',
        noDuplicates: (value) => {
          if (!value) return true
          const sentences = value.map((v) => v.original_sentence.trim())
          const uniqueSentences = new Set(sentences)
          return sentences.length === uniqueSentences.size || 'Các câu gốc không được trùng nhau'
        },
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center pb-2 border-b border-gray-200">
            <div>
              <h4 className="text-sm font-medium text-[#2D3748]">Các câu hoàn thành</h4>
            ...
            {fields.map((field, index) => (
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  <h5 className="text-sm font-medium text-[#2D3748]">Câu hoàn thành {index + 1}</h5>
                  {fields.length > 1 && (
                    <Button
                      type="button"
                ...
                <div className="space-y-4">
                  ...
                  <FormField
                    ...
                    render={({ field: originalField, fieldState: { error } }: RenderProps) => (
                      <FormItem>
                        <FormLabel className="text-[#2D3748] font-medium">Câu gốc</FormLabel>
                        <FormControl>
                          <textarea
                            {...originalField}
                  ...
                  <FormField
                    ...
                    render={({ field: startField }: RenderProps) => (
                      <FormItem>
                        <FormLabel className="text-[#2D3748] font-medium">Phần bắt đầu</FormLabel>
                        <FormControl>
                          <textarea
                            {...startField}
                  ...
                  <FormField
                    ...
                    render={({ field: middleField }: NullableRenderProps) => (
                      <FormItem>
                        <FormLabel className="text-[#2D3748] font-medium">Phần giữa</FormLabel>
                        <FormControl>
                          <textarea
                            {...middleField}
                  ...
                  <FormField
                    ...
                    render={({ field: endField }: NullableRenderProps) => (
                      <FormItem>
                        <FormLabel className="text-[#2D3748] font-medium">Phần cuối</FormLabel>
                        <FormControl>
                          <textarea
                            {...endField}
                    ...
                    rules={{
                      ...
                      validate: {
                        hasValidSentence: (value: string[]) => {
                          ...
                          const validSentences = value.filter((s) => s && s.trim().length > 0)
                          return validSentences.length > 0 || 'Vui lòng nhập ít nhất một câu đúng'
                        }
                      }
                    }}
                    render={({
                      ...
                      <FormItem>
                        <div className="flex justify-between items-center">
                          ...
                          <Button
                            type="button"
                            onClick={() => {
                              const newSentences = [...(trueSentencesField.value || []), '']
                        ...
                        <FormControl>
                          <div className="space-y-2">
                            {(trueSentencesField.value || ['']).map((sentence, sentenceIndex) => (
                              <div key={sentenceIndex} className="flex gap-2">
                                <textarea
                                  ...
                                  onChange={(e) => {
                                    ...
                                    trueSentencesField.onChange(newSentences)
                                    form.trigger(`sentence_completions.${index}.true_sentences`)
                                  }}
                                  placeholder={`Câu đúng ${sentenceIndex + 1}`}
                                ...
                                {trueSentencesField.value.length > 1 && (
                                  <Button
                                    type="button"
                                    onClick={() => {
                                      const newSentences = trueSentencesField.value.filter(
                                        (_, i) => i !== sentenceIndex
                                      )
                                      trueSentencesField.onChange(newSentences)
                                      form.trigger(`sentence_completions.${index}.true_sentences`)
